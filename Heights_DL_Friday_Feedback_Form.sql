/*This file data set is sent to DeansList every thursday evening for use in the friday feedback form*/
...
LEFT JOIN (
		...
		CASE -- THIS ASSIGNS THE DATE OF THE FRIDAY OF THE WEEK THAT THE LESSON WAS PASSED SO THEY CAN BY GROUPED BY WEEK
			WHEN DATEPART(DW,CAST(I.[COMPLETION DATE] AS DATE))<=6 THEN DATEADD(D,6-DATEPART(DW,CAST(I.[COMPLETION DATE] AS DATE)),CAST(I.[COMPLETION DATE] AS DATE))
			WHEN DATEPART(DW,CAST(I.[COMPLETION DATE] AS DATE))=7 THEN DATEADD(D,6 ,CAST(I.[COMPLETION DATE] AS DATE))
			ELSE NULL 
		END TESTDATE,
		SUM(CAST([TIME ON TASK (MINUTES)] AS INT)) TIME_ON_TASK
		FROM
		[CUST1220].[TEST_IREADY].[TEST_IREADY_STUDENT_LESSON_ACTIVITY] I
		GROUP BY (CASE -- THIS ASSIGNS THE DATE OF THE FRIDAY OF THE WEEK THAT THE LESSON WAS PASSED SO THEY CAN BY GROUPED BY WEEK
					WHEN DATEPART(DW,CAST(I.[COMPLETION DATE] AS DATE))<=6 THEN DATEADD(D,6-DATEPART(DW,CAST(I.[COMPLETION DATE] AS DATE)),CAST(I.[COMPLETION DATE] AS DATE))
					WHEN DATEPART(DW,CAST(I.[COMPLETION DATE] AS DATE))=7 THEN DATEADD(D,6 ,CAST(I.[COMPLETION DATE] AS DATE))
					ELSE NULL 
				  END),[STUDENT ID],school) I ON cast(I.[STUDENT ID] as int) = S.STUDENT_NUMBER AND I.TESTDATE = @REPORTDATE
